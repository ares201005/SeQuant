...
#include <SeQuant/domain/mbpt/models/qedcc.hpp>
...
QEDCC::QEDCC(size_t n, size_t p, size_t pmin)
    : N(n), P(p == std::numeric_limits<size_t>::max() ? n : p), PMIN(pmin) {}

ExprPtr QEDCC::sim_tr(ExprPtr expr, size_t r) {
  ...
  if (expr.is<op_t>()) {
    ...
    throw std::invalid_argument(
        "QEDCC::sim_tr(expr): Unsupported expression type");
}

std::vector<ExprPtr> QEDCC::t(bool screen, bool use_topology,
...
std::vector<ExprPtr> QEDCC::λ(bool screen, bool use_topology,
                           bool use_connectivity, bool canonical_only) {
  // construct hbar
